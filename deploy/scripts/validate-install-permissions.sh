...
BACKUP_USER="${BACCARAT_BACKUP_USER:-baccarat-backup}"
BACKUP_DIR="${BACCARAT_BACKUP_DIR:-/var/backups/baccarat}"
...
check_not_writable_by_runtime() {
  ...
  for user in "$API_USER" "$WORKER_USER" "$BACKUP_USER"; do
...
id "$BACKUP_USER" >/dev/null 2>&1 || fail "runtime user $BACKUP_USER exists"
...
if require_path "$BACKUP_DIR"; then
  check_owner_group "$BACKUP_DIR" "$BACKUP_USER" "$CONFIG_GROUP"
  check_mode "$BACKUP_DIR" 700
  if runuser -u "$BACKUP_USER" -- test -w "$BACKUP_DIR"; then
    pass "$BACKUP_DIR is writable by $BACKUP_USER"
  else
    fail "$BACKUP_DIR is not writable by $BACKUP_USER"
  fi
  for user in "$API_USER" "$WORKER_USER"; do
    if runuser -u "$user" -- test -w "$BACKUP_DIR"; then
      fail "$BACKUP_DIR is writable by $user"
    else
      pass "$BACKUP_DIR is not writable by $user"
    fi
  done
fi
