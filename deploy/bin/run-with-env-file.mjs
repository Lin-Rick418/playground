...
const ALLOWED_MODES = new Set([0o400, 0o440, 0o600, 0o640]);
...
export function validateEnvFileMetadata(metadata, effectiveUid, effectiveGroupIds = []) {
  if (!metadata.isFile()) {
  ...
  if (!ALLOWED_MODES.has(mode)) {
    throw new SafeEnvError("Environment file mode must be 0400, 0440, 0600, or 0640");
  }

  const groupIds = new Set(effectiveGroupIds);
  const ownerReadable = metadata.uid === effectiveUid && (mode & 0o400) !== 0;
  const groupReadable = groupIds.has(metadata.gid) && (mode & 0o040) !== 0;

  if (!ownerReadable && !groupReadable) {
    throw new SafeEnvError("Environment file must be owned by the command user or its readable config group");
  }
...
export async function loadEnvFile(
  ...
  effectiveGroupIds = [process.getegid?.(), ...(process.getgroups?.() ?? [])].filter(
  ...
  try {
    ...
    validateEnvFileMetadata(metadata, effectiveUid, effectiveGroupIds);
