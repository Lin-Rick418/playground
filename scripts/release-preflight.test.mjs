...
const config = { authoritativeBranch: "master", remote: "origin" };
...
test("rejects a branch other than the configured authoritative branch", () => {
  assertBlocked({ "symbolic-ref --quiet --short HEAD": "feature/release" }, /expected master/);
});
...
test("rejects a missing or unexpected upstream", () => {
  ...
  assertBlocked(
    { "rev-parse --abbrev-ref --symbolic-full-name @{upstream}": "fork/master" },
