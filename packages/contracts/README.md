...
Covered contracts include auth login/current user, paginated admin users, lobby/table state, and WebSocket subscription/snapshot messages.
...
The admin users endpoint now returns an envelope:
...
This replaces the previous unbounded array response. Server and web must therefore be deployed from the same release. Future breaking schema changes should use an explicit compatibility window or API version instead of changing an existing contract in place.
