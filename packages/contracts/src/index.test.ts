...
describe("shared API contracts", () => {
  it("validates auth requests and responses strictly", () => {
    ...
    assert.equal(loginResponseSchema.parse({ token: "jwt", user }).user.role, "PLAYER");
    assert.equal(loginRequestSchema.safeParse({ username: "player1", password: "secret", role: "ADMIN" }).success, false);
    assert.equal(loginResponseSchema.safeParse({ token: "jwt", user: { ...user, role: "OWNER" } }).success, false);
  });

  it("coerces and bounds admin users pagination", () => {
    assert.deepEqual(adminUsersQuerySchema.parse({}), { page: 1, pageSize: 20 });
    assert.deepEqual(adminUsersQuerySchema.parse({ page: "2", pageSize: "50" }), { page: 2, pageSize: 50 });
    assert.equal(adminUsersQuerySchema.safeParse({ page: "0", pageSize: "101" }).success, false);
    assert.equal(
      adminUsersResponseSchema.safeParse({
        items: [{ ...user, createdAt: now }],
