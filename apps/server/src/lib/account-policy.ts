import { z } from "zod";
import { betTypes, type BetType, type UserRecord } from "../types/domain.js";

export const MONEY_DENOMINATION = 100;
export const MAX_ACCOUNT_BALANCE = 2_000_000_000;
export const MAX_INITIAL_BALANCE = 100_000_000;
export const MAX_BALANCE_ADJUSTMENT = 100_000_000;
...
export const createPlayerSchema = z
  .object({
    ...
    balance: moneyAmountSchema(0, MAX_INITIAL_BALANCE).default(0),
  })
  .strict()
  .superRefine((input, context) => {
    const violation = getPasswordPolicyViolation(input.password, input.username);
    if (violation) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ["password"], message: violation });
    }
  });
...
const optionalNoteSchema = z
  .string()
  .max(200)
  .refine((note) => !/[\u0000-\u001f\u007f]/.test(note), "Control characters are not allowed")
...
export const adjustBalanceSchema = z
  .object({
    userId: resourceIdSchema,
    amount: moneyAmountSchema(-MAX_BALANCE_ADJUSTMENT, MAX_BALANCE_ADJUSTMENT).refine(
      (amount) => amount !== 0,
...
export const resourceIdParamsSchema = z.object({ id: resourceIdSchema }).strict();
...
export const liveClientMessageSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("subscribe_table"), tableId: resourceIdSchema }).strict(),
]);
...
export function wouldRemoveLastActiveAdmin(
  user: Pick<UserRecord, "role" | "isActive">,
  nextIsActive: boolean,
  activeAdminCount: number,
) {
  return user.role === "ADMIN" && user.isActive && !nextIsActive && activeAdminCount <= 1;
}

export type SafeUser = Pick<UserRecord, "id" | "username" | "role" | "isActive" | "balance">;
export type SafeAdminUser = SafeUser & Pick<UserRecord, "createdAt">;
...
export function toSafeAdminUser(user: UserRecord): SafeAdminUser {
  return { ...toSafeUser(user), createdAt: user.createdAt };
}
