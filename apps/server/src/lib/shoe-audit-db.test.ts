...
test("persists active secrets across restart and immutable proof after rotation", { skip: !testDatabaseUrl }, async () => {
  ...
  try {
    ...
    await db.withTransaction(async (client) => {
      await db.updateUserBalance(mismatchPlayer.id, 900, client);
      await db.createBet(
    ...
    const insufficientPlayer = await db.createPlayer({
      username: `insufficient-${randomUUID()}`,
      passwordHash: "integration-test-only",
      balance: 1_000,
    });
    ...
    await db.withTransaction(async (client) => {
      await db.updateUserBalance(insufficientPlayer.id, 800, client);
      await db.createBet(
    ...
    await db.withTransaction(async (client) => {
      await db.updateUserBalance(legacyPlayer.id, 700, client);
      await db.createBet(
