...
import {
  assertDatabaseSchemaContract,
  assertUnmanagedSchemaAdoptable,
...
export async function getMigrationStatus(
  ...
  if (pending.length === 0 && catalog === migrationCatalog) {
    await assertDatabaseSchemaContract(executor);
  }
...
export async function runMigrations(
  ...
  try {
    ...
    if (!existingHistory && definitions === migrationDefinitions) {
      await assertUnmanagedSchemaAdoptable(client);
    }
    ...
    if (definitions === migrationDefinitions) {
      await assertDatabaseSchemaContract(client);
    }
