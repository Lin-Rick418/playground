...
test("Plinko real API settles once, persists history and keeps exact wallet cents", async ({
  page,
}) => {
  ...
  try {
    ...
    const responsePromise = page.waitForResponse(
      (response) =>
        response.request().method() === "POST" && response.url().endsWith("/plinko/rounds"),
    );
    await page.getByRole("button", { name: "投球", exact: true }).click();
    const response = await responsePromise;
    expect(response.status()).toBe(200);
    const result = (await response.json()) as PlinkoMutationResponse;
    ...
    await expect(page).toHaveURL(/\/history\?game=plinko$/);
    await expect(
      page.getByText(result.round.multiplier.toFixed(2) + "×", { exact: true }).first(),
    ).toBeVisible();
...
test("Plinko recovers a committed but lost response with the same key after reload", async ({
  ...
  try {
    ...
    const keys: string[] = [];
    let requests = 0;
    await page.route("**/api/plinko/rounds", async (route) => {
      keys.push(route.request().headers()["idempotency-key"]!);
      requests++;
      if (requests === 1) {
        const response = await route.fetch();
        expect(response.status()).toBe(200);
        committed = (await response.json()) as PlinkoMutationResponse;
